...
def load_fertilizer_models():
    ...
    if model is not None and ferti_encoder is not None:
        return
    try:
        if _os.path.exists('classifier.pkl'):
            with open('classifier.pkl', 'rb') as f:
                model = pickle.load(f)
        if _os.path.exists('fertilizer.pkl'):
            with open('fertilizer.pkl', 'rb') as f:
                ferti_encoder = pickle.load(f)
    except Exception:
        # If loading fails, leave models as None and handle at request time
        model = None
        ferti_encoder = None
...
def fertilizer_recommendation():
    if request.method == 'POST':
        ...
        # Ensure ML models are loaded (lazy load)
